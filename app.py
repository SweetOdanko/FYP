...
def get_db_connection():
    db_url = os.getenv('DATABASE_URL')  # Fetch the MongoDB connection string from environment variables
    if not db_url:
        raise ValueError("No DATABASE_URL found in environment variables")
    client = MongoClient(db_url)
    return client['image_search_engine']  # Replace with your actual database name
...
def find_similar_images(features, collection):
    ...
    if similarities[0][1] >= similarity_threshold:
        # Retrieve the first image and the next five images
        similar_images = [str(similarities[i][0]) for i in range(6)]
    else:
        # Retrieve images that meet or exceed the threshold
        similar_images = [str(similarity[0]) for similarity in similarities if similarity[1] >= similarity_threshold]
        similar_images = similar_images[:6]  # Ensure we return at most 6 images
